## 0.2.1
- Включён звук для онлайн-просмотра камер во встроенном плеере.

## 0.2.0
- Исправлена работа с актуальными `accessControls` и `externalCameraId`: снимки, видеопотоки, открытие дверей и YAML-конфигурация Home Assistant.
- Исправлено обновление токенов и состояние готовности API.
- Добавлен просмотр архивных записей и онлайн-потоков через `mpegts.js`.
- Для архивных событий элементы просмотра отображаются только в пределах трёх суток хранения.
- Исправлен вывод всех событий и их размещение в карточке.
- В YAML добавлены поясняющие комментарии для камер и открытия дверей.
